...
Live site: **https://lkbell.github.io/sit-rep/** — 72 indicator charts, 10 domains + an Overview page of 16 vital signs with a signals strip. Purpose: Landon's ground-truth situational awareness. Data auto-refreshes via GitHub Actions (weekdays 14:23 UTC + on any pipeline push); the site is pure static files on GitHub Pages.
...
- `explainers.json` — static "What it measures / Why it matters / What noteworthy looks like" text, ALL 72 chart ids. The ⓘ button appears automatically for any chart id present here.
...
- **CBP monthly border encounters** (`src_cbp_monthly` / `border_monthly`, added 2026-07-08): SELF-DISCOVERING — scrapes the CBP Public Data Portal page `document/stats/nationwide-encounters` for the "Nationwide Encounters by Area of Responsibility" (`-aor`) CSVs, keeps the newest file per fiscal-year range (the rolling `fy23-fy26-<mon>` file + the FY20/FY21/FY22 archives), merges them (newer revisions win) and sums Encounter Count to monthly totals. Two series: Nationwide (all rows) and Southwest land border (the "Southwest Land Border" region rows). Fiscal-year label may read e.g. `2026 (FYTD)`; fiscal Oct–Dec map to the prior calendar year. Validation: the independent `-aor` and `-state` breakdowns agree to the unit on the nationwide total (e.g. FY2023 = 3,148,444), so this reproduces CBP’s published figures. NOTE: the SW cut of this dataset runs ~1% below the standalone SW product (`document/stats/southwest-land-border-encounters`, e.g. FY2023 2,451,355 vs 2,475,669) — both series here come from the one Nationwide dataset for internal consistency. If CBP renames the `-aor` files or drops the region column, the adapter raises with the header it saw.
