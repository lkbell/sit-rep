...
SECTIONS = [
    ...
    ("demographics", "Demographics, immigration & education", "Who and what America is today, and how it has changed: population, its makeup, immigration, and schooling."),
    ("crime", "Crime & justice", "Violent and property crime, how many killings are solved, and incarceration."),
...
CATALOG = {
    ...
    "hy_oas": dict(sec="markets", title="High-yield credit spread (OAS)", unit="pp", fmt="pct", dec=2, freq="d", rng=15,
                   ...
                   note="Stress gauge: widening spreads = markets pricing more default risk. Note: FRED now serves only ~3 years of this ICE index; see 'Credit stress, long-run (Baa spread)' for the multi-decade view."),
    "baa_spread": dict(sec="markets", title="Credit stress, long-run (Baa spread)", unit="pp", fmt="pct", dec=2, freq="d", rng="max",
                       series=[dict(name="Baa corporate spread over 10Y", src=["fred", "BAA10Y"])],
                       source_name="Moody's / FRED", source_url=F + "BAA10Y",
                       note="The extra yield investors demand to hold medium-grade (Baa) corporate bonds over Treasuries -- the long-run companion to the high-yield spread (which FRED now caps at ~3 years). It widens in every recession. Daily since 1986."),
    ...
    # ---- immigration ----
    "swb_fy": dict(sec="demographics", title="Southwest border apprehensions by fiscal year (USBP)", unit="count", fmt="num", dec=0, freq="manual", rng="max",
    ...
    "consumer_debt": dict(sec="economy", title="Household debt balances", unit="$T", fmt="usd", dec=2, freq="m", rng="max",
                          series=[dict(name="Total consumer credit", src=["fred", "TOTALSL"], tf=[["scale", 1e-3]]),
                                  dict(name="Credit cards (revolving)", src=["fred", "REVOLSL"], tf=[["scale", 1e-3]]),
                                  dict(name="Auto and other (nonrevolving)", src=["fred", "NONREVSL"], tf=[["scale", 1e-3]])],
                          source_name="Federal Reserve G.19 via FRED", source_url=F + "TOTALSL",
                          note="What households owe outside their mortgage. Revolving is mostly credit cards; nonrevolving is mostly auto and student loans. Balances grow with the economy -- watch the pace, and read alongside the debt-service burden."),
    "debt_burden": dict(sec="economy", title="Household debt-service burden", unit="%", fmt="pct", dec=1, freq="q", rng="max", exp=290,
                        series=[dict(name="Debt payments as % of disposable income", src=["fred", "TDSP"])],
                        source_name="Federal Reserve via FRED", source_url=F + "TDSP",
                        note="Share of after-tax income households must put toward debt payments -- the cleanest read on whether debt loads are affordable. It climbed ahead of both the dot-com and 2008 downturns. Quarterly, since 1980."),
    "violent_crime": dict(sec="crime", title="Violent crime rate", unit="per 100k", fmt="num", dec=0, freq="manual", rng="max", exp=550,
                      series=[dict(name="Violent crime", src=["manual", "violent_crime"])],
                      source_name="FBI UCR", source_url="https://cde.ucr.cjis.gov/",
                      note="Reported violent crimes per 100,000 people. Curated from FBI Uniform Crime Reporting; updated each fall."),
    "property_crime": dict(sec="crime", title="Property crime rate", unit="per 100k", fmt="num", dec=0, freq="manual", rng="max", exp=550,
                      series=[dict(name="Property crime", src=["manual", "property_crime"])],
                      source_name="FBI UCR", source_url="https://cde.ucr.cjis.gov/",
                      note="Reported property crimes (burglary, theft, motor-vehicle theft) per 100,000 people. Curated from FBI UCR; updated each fall."),
    "homicide_clearance": dict(sec="crime", title="Homicide clearance rate", unit="%", fmt="pct", dec=0, freq="manual", rng="max", exp=550,
                      series=[dict(name="Homicides cleared", src=["manual", "homicide_clearance"])],
                      source_name="FBI UCR / Murder Accountability Project", source_url="https://www.murderdata.org/",
                      note="Share of murders cleared by arrest or exceptional means. Curated from FBI UCR (consistent MAP series); updated each fall."),
    "incarceration": dict(sec="crime", title="Imprisonment rate", unit="per 100k", fmt="num", dec=0, freq="manual", rng="max", exp=700,
                      series=[dict(name="Imprisonment rate", src=["manual", "incarceration"])],
                      source_name="Bureau of Justice Statistics", source_url="https://bjs.ojp.gov/",
                      note="Sentenced state and federal prisoners per 100,000 U.S. residents (excludes local jails). Curated from BJS."),
    "race_ethnicity": dict(sec="demographics", title="Population by race & ethnicity (share)", unit="%", fmt="pct", dec=1, freq="manual", rng="max", exp=2000,
                      series=[dict(name="White (non-Hispanic)", src=["manual", "race_white"]),
                              dict(name="Hispanic", src=["manual", "race_hispanic"]),
                              dict(name="Black", src=["manual", "race_black"]),
                              dict(name="Asian", src=["manual", "race_asian"])],
                      source_name="U.S. Census Bureau", source_url="https://www.census.gov/topics/population/race.html",
                      note="Each group as a share of total U.S. population; Hispanic is an ethnicity counted separately from race, so shares do not sum to 100. Census decennial + latest estimates. 1970 figures are sample-based and pre-2000 Asian includes Pacific Islanders."),
    "foreign_born": dict(sec="demographics", title="Foreign-born share of population", unit="%", fmt="pct", dec=1, freq="manual", rng="max", exp=550,
                      series=[dict(name="Foreign-born", src=["manual", "foreign_born"])],
                      source_name="U.S. Census Bureau / ACS", source_url="https://www.census.gov/topics/population/foreign-born.html",
                      note="Share of U.S. residents born outside the country. The 2024 level (~14.8%) essentially ties the all-time high of 14.8% set in 1890."),
    "pop_growth": dict(sec="demographics", title="Population growth rate", unit="%", fmt="pct", dec=2, freq="a", rng="max", zero_line=True, exp=800,
                      series=[dict(name="Annual growth", src=["fred", "SPPOPGROWUSA"])],
                      source_name="World Bank via FRED", source_url=F + "SPPOPGROWUSA",
                      note="Annual U.S. population growth. It has trended down for decades toward record lows, and is now driven mostly by immigration rather than births."),
    "natural_increase": dict(sec="demographics", title="Natural increase (births minus deaths)", unit="M/yr", fmt="num", dec=2, freq="manual", rng="max", zero_line=True, exp=550,
                      series=[dict(name="Natural increase", src=["manual", "natural_increase"])],
                      source_name="CDC NCHS", source_url="https://www.cdc.gov/nchs/nvss/deaths.htm",
                      note="U.S. births minus deaths per year, in millions. It collapsed from about 2 million in 1990 toward 0.2 million in 2021 as births fell and deaths rose - the reason population growth now hinges on immigration."),
    "age_65plus": dict(sec="demographics", title="Share of population age 65+", unit="%", fmt="pct", dec=1, freq="a", rng="max", exp=800,
                      series=[dict(name="65 and older", src=["fred", "SPPOP65UPTOZSUSA"])],
                      source_name="World Bank via FRED", source_url=F + "SPPOP65UPTOZSUSA",
                      note="Americans 65+ as a share of the population - roughly 10% in 1970 to about 18% now as the Baby Boomers age. Drives entitlement costs and the shrinking worker-to-retiree ratio."),
    "naep_reading": dict(sec="demographics", title="Reading scores (NAEP), ages 9 & 13", unit="points", fmt="num", dec=0, freq="manual", rng="max", exp=1500,
                      series=[dict(name="Age 9", src=["manual", "naep_read9"]),
                              dict(name="Age 13", src=["manual", "naep_read13"])],
                      source_name="NAEP Long-Term Trend (NCES)", source_url="https://www.nationsreportcard.gov/ltt/",
                      note="NAEP Long-Term Trend reading scale scores (0-500) - actual test results, not attainment. Both ages slid after 2012, with the largest drops among lower-performing students."),
    "naep_math": dict(sec="demographics", title="Math scores (NAEP), ages 9 & 13", unit="points", fmt="num", dec=0, freq="manual", rng="max", exp=1500,
                      series=[dict(name="Age 9", src=["manual", "naep_math9"]),
                              dict(name="Age 13", src=["manual", "naep_math13"])],
                      source_name="NAEP Long-Term Trend (NCES)", source_url="https://www.nationsreportcard.gov/ltt/",
                      note="NAEP Long-Term Trend math scale scores (0-500). Age-13 math fell 14 points from its 2012 peak to 2023, back to 1990s levels."),
    "chronic_absenteeism": dict(sec="demographics", title="Chronic absenteeism", unit="%", fmt="pct", dec=1, freq="manual", rng="max", exp=550,
                      series=[dict(name="Chronically absent", src=["manual", "absenteeism"])],
                      source_name="U.S. Dept. of Education / AEI Return to Learn", source_url="https://www.returntolearntracker.net/",
                      note="Share of public-school students missing at least 10% of school days. It nearly doubled to about 28% after COVID (2021-22) and is receding only slowly."),
    "pisa_math": dict(sec="demographics", title="Math: U.S. vs. OECD (PISA)", unit="points", fmt="num", dec=0, freq="manual", rng="max", exp=1400,
                      series=[dict(name="United States", src=["manual", "pisa_us"]),
                              dict(name="OECD average", src=["manual", "pisa_oecd"])],
                      source_name="OECD PISA", source_url="https://www.oecd.org/pisa/",
                      note="Mean math score of 15-year-olds, every three years. U.S. 2022 (465) was its lowest ever; it now sits near the OECD average largely because the OECD average itself fell."),
}
...
CATALOG["border_monthly"] = dict(sec="demographics", title="Border encounters by month (nationwide vs. Southwest)", unit="encounters", fmt="num", dec=0, freq="m", rng="max", exp=100,
